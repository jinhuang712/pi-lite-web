...
/** Presentation slots pi-briefly takes over when it decorates the row. */
...
export interface ToolRowDecoration {
	...
	renderResult?: RowCallRenderer;
	renderShell?: "default" | "self";
	// Loosely typed so the spread stays assignable to the owner's typed definition.
	parameters?: any;
	prepareArguments?: (args: any) => any;
}
 ...
 * may have changed.
 *
 * The re-apply has to happen in `session_start` rather than at load time: every
 ...
 * which extension the user installed first. `apply` must re-register the tool,
 * because that is the only way a presentation change reaches Pi.
 */
